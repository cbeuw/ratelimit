...
func (rateLimitSuite) TestTake(c *gc.C) {
	for i, test := range takeTests {
		tb := NewLimitedBucket(test.fillInterval, test.capacity)
...
func (rateLimitSuite) TestPanics(c *gc.C) {
	c.Assert(func() { NewLimitedBucket(0, 1) }, gc.PanicMatches, "token bucket fill interval is not > 0")
	c.Assert(func() { NewLimitedBucket(-2, 1) }, gc.PanicMatches, "token bucket fill interval is not > 0")
	c.Assert(func() { NewLimitedBucket(1, 0) }, gc.PanicMatches, "token bucket capacity is not > 0")
	c.Assert(func() { NewLimitedBucket(1, -2) }, gc.PanicMatches, "token bucket capacity is not > 0")
}
...
func (rateLimitSuite) TestRate(c *gc.C) {
	tb := NewLimitedBucket(1, 1)
	if !isCloseTo(tb.Rate(), 1e9, 0.00001) {
		c.Fatalf("got %v want 1e9", tb.Rate())
	}
	tb = NewLimitedBucket(2*time.Second, 1)
	if !isCloseTo(tb.Rate(), 0.5, 0.00001) {
		c.Fatalf("got %v want 0.5", tb.Rate())
	}
	tb = NewLimitedBucketWithQuantum(100*time.Millisecond, 1, 5)
...
func checkRate(c *gc.C, rate float64) {
	tb := NewLimitedBucketWithRate(rate, 1<<62)
...
func TestAvailable(t *testing.T) {
	for i, tt := range availTests {
		tb := NewLimitedBucket(tt.fillInterval, tt.capacity)
...
func BenchmarkNewBucket(b *testing.B) {
	for i := b.N - 1; i >= 0; i-- {
		NewLimitedBucketWithRate(4e18, 1<<62)
	}
}
